        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-600">
              <div className="text-white font-bold text-xl">记忆</div>
              <button
            ...
            <div className="flex-1 min-h-0 flex overflow-hidden">
              ...
              <div className="w-40 border-r border-gray-600 overflow-y-auto p-2 space-y-1">
                {entries.length === 0 ? (
                  <div className="text-gray-500 text-sm text-center py-4">暂无记忆</div>
                ) : (
                  entries.map((entry) => (
                    <button
                      key={entry.key}
                ...
                {selectedEntry ? (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="text-gray-400 text-sm">{selectedEntry.title}</div>
                      <div className="text-gray-500 text-xs">{selectedEntry.updatedAt}</div>
                    </div>
                    ...
                    <div className="bg-gray-700/50 rounded-lg p-4 space-y-4 border border-gray-600/50">
                      <div>
                        ...
                        {selectedEntry.data.gender ? (
                          <span className="inline-block mt-1 text-xs px-2 py-0.5 rounded-full bg-blue-600/30 text-blue-200 border border-blue-500/30">
                            {selectedEntry.data.gender}
                          </span>
                        ) : null}
                      </div>
                      ...
                      {selectedEntry.data.appearance ? (
                        <div>
                          <div className="text-sm font-semibold text-yellow-500 mb-1">外貌</div>
                          <div className="text-base text-gray-200 leading-relaxed">{selectedEntry.data.appearance}</div>
                        </div>
                      ) : null}

                      {selectedEntry.data.personality ? (
                        <div>
                          <div className="text-sm font-semibold text-green-500 mb-1">性格</div>
                          <div className="text-base text-gray-200 leading-relaxed">{selectedEntry.data.personality}</div>
                        </div>
                      ) : null}

                      {selectedEntry.data.backstory ? (
                        <div>
                          <div className="text-sm font-semibold text-purple-500 mb-1">背景</div>
                          <div className="text-base text-gray-200 leading-relaxed">{selectedEntry.data.backstory}</div>
                        </div>
                      ) : null}
                    </div>
                  </div>
                ) : (
                  <div className="text-gray-500 text-base text-center py-8">暂无记忆内容</div>
                )}
              </div>
            </div>
          </div>
